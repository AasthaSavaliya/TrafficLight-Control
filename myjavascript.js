   ...
   function createFile() {
      if ($("input[name=mode]:checked").val() == "timeBased") {
         ...
         let downloadButton = $("#file_download");

         downloadButton.show();


         let redNS = $("#redNS").val(),
            yellowNS = $("#yellowNS").val(),
            greenNS = $("#greenNS").val(),
            redEW = $("#redEW").val(),
            yellowEW = $("#yellowEW").val(),
            greenEW = $("#greenEW").val();

         let xmlString = `<main>
            <RedNS>${redNS}</RedNS>
            <YellowNS>${yellowNS}</YellowNS>
            <GreenNS>${greenNS}</GreenNS>

            <RedEW>${redEW}</RedEW>
            <YellowEW>${yellowEW}</YellowEW>
            <GreenEW>${greenEW}</GreenEW>
         </main>`;

         let fileName = "file.xml";
         let bb = new Blob([xmlString], {type: 'text/plain'});

         downloadButton.attr('href', window.URL.createObjectURL(bb));
